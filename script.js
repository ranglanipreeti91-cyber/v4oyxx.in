...
function initMedia() {
  ...
  backgroundMusic.muted = true; // start muted for mobile autoplay policies
  backgroundVideo.muted = true;

  // Preload all audio elements so they are ready after user gesture
  const allAudio = [
    backgroundMusic,
    document.getElementById('hacker-music'),
    document.getElementById('rain-music'),
    document.getElementById('anime-music'),
    document.getElementById('car-music')
  ].filter(Boolean);
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Declared early so startExperience (mobile audio unlock) can use them
  let currentAudio = backgroundMusic;
  let isMuted = false;

  // Robust mobile-friendly audio unlock + start experience
  // Mobile browsers (esp. iOS Safari) only allow audio after a direct user gesture.
  const allAudioElements = [backgroundMusic, hackerMusic, rainMusic, animeMusic, carMusic].filter(Boolean);

  function unlockAllAudio() {
    // Classic iOS unlock: briefly play+pause muted tracks so later play() works
  ...
  function playAudioSafely(audio) {
    ...
    if (playPromise !== undefined) {
      playPromise.catch(err => {
        ...
        // One quick retry after short delay (helps some mobile browsers)
        setTimeout(() => {
  ...
  function startExperience(e) {
    ...
    // Unlock audio context / elements first (critical for mobile)
    unlockAllAudio();
  ...
  // currentAudio & isMuted declared earlier for mobile startExperience
