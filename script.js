...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startBackgroundMusicFromGesture() {
    backgroundMusic.muted = false;
    backgroundMusic.volume = parseFloat(volumeSlider.value) || 0.3;
    backgroundMusic.load();

    const playPromise = backgroundMusic.play();
    if (playPromise && typeof playPromise.catch === 'function') {
      playPromise.catch(err => {
        console.warn("Mobile audio playback was blocked; retrying on next user gesture:", err);
      });
    }
  }
  ...
  function enterSite(e) {
    ...
    // IMPORTANT: play() is called directly from the user's touch/pointer gesture.
  ...
  // One handler works for mouse, touch, and pen without firing twice on mobile.
  startScreen.addEventListener('pointerdown', enterSite, { passive: false });

  /* MOBILE AUDIO FALLBACK
     If a browser still rejects playback, the next real user gesture retries it. */
  document.addEventListener('pointerdown', () => {
    if (backgroundMusic.paused && !startScreen.classList.contains('hidden')) {
      startBackgroundMusicFromGesture();
    }
  }, { passive: true });
  const name = "V4oyxx!";
  ...
  const bioMessages = [
    "Python & C++ dev | Discord bot creator.",
    "\"Hello, World!\""
  ];
  let bioText = '';
  let bioIndex = 0;
  ...
  function switchTheme(videoSrc, audio, themeClass, overlay = null, overlayOverProfile = false) {
    ...
    gsap.to(backgroundVideo, {
      ...
      onComplete: () => {
        ...
        currentAudio = audio;
        currentAudio.volume = parseFloat(volumeSlider.value) || 0.3;
        currentAudio.muted = isMuted;

        // Theme switches are also user gestures on mobile, so playback is allowed.
        const themePlay = currentAudio.play();
        if (themePlay && typeof themePlay.catch === 'function') {
          themePlay.catch(err => console.error("Failed to play theme music:", err));
        }
